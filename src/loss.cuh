...
__global__ void categoricalCrossEntropyKernel(float *y, float *y_pred, float *error, int n, int classes)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if (idx < n)
    {
        ...
        for (int c = 0; c < classes; c++)
        {
            ...
            loss -= y[offset] * logf(y_pred[offset] + 1e-10); // Added epsilon to avoid log(0)
        }
        error[idx] = loss;
    }
}
